...
class OrderViewSet(viewsets.ViewSet):
    ...
    Обрабатывает создание, просмотр и удаление заказов с автоматическим
    управлением остатками товаров на складе.
    """
    ...
    def list(self, request: Request) -> Response:
        ...
        Returns:
            Response: Список заказов или пустой массив для неавторизованных.
        """
        user = request.user
    ...
    def create(self, request: Request) -> Response:
        ...
        Проверяет наличие товаров на складе, создает заказ и атомарно
        уменьшает остатки. Использует транзакции и блокировки для
        ...
        Returns:
            Response: Данные созданного заказа и URL оплаты или ошибка
                     с детальной информацией о недостающих товарах.
        """
    ...
    def retrieve(self, request: Request, pk) -> Response:
        ...
        Returns:
            Response: Данные заказа или 403 если заказ принадлежит другому пользователю.
        """
        user: AbstractUser | AnonymousUser = request.user
    ...
    def delete(self, request: Request, pk) -> Response:
        ...
        Остатки восстанавливаются только для заказов в статусе
        'pending_payment' или 'pending_assembly'.
        ...
        Returns:
            Response: 204 при успешном удалении или 403 если заказ
    ...
    def check_payment_status(self, request: Request, pk) -> Response:
        ...
        При успешной оплате обновляет статус заказа на 'pending_assembly'.
        При неудачной оплате восстанавливает остатки товаров и удаляет заказ.
        
        Args:
            request: HTTP запрос с параметром bankOrderId.
            pk: ID заказа.
            
        Returns:
            Response: Статус оплаты ('successful' или 'failure').
        """
